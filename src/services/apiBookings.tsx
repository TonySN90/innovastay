...
export async function getBookings(): Promise<IBookingTypes[] | null> {
  const { data: bookings, error }: PostgrestSingleResponse<IBookingTypes[]> =
    await supabase
      .from("bookings")
      .select(
        "id, created_at, startDate, endDate, numNights, numGuests, status, totalPrice, isPaid, hasBreakfast, cabins(name, id, image, category, price), guests(id, fullName, address, postalCode, city, country, email, phone, information)",
        { count: "exact" }
      );
